...
$$
p_i = \frac{e^{z_i}}{\sum_{j=1}^{k} e^{z_j}} \quad \mathrm{for}\; i = 1, 2, \ldots, k
$$
...
$$
\mathrm{softmax}(z) = [p_1, p_2, p_3] = \hat{y}
$$
...
```text
softmax(z) = [p_virginica, p_setosa, p_versicolor]
```
...
| 가설식 | $H(X)=\mathrm{sigmoid}(WX+B)$ | $H(X)=\mathrm{softmax}(WX+B)$ |
...
$$
\mathrm{cost}(W) = -\sum_{j=1}^{k} y_j \log(p_j)
$$
...
$$
\mathrm{cost}(W) = -[0\log(0.26) + 1\log(0.71) + 0\log(0.03)]
$$

$$
\mathrm{cost}(W) = -\log(0.71)
$$
...
$$
\mathrm{cost}(W) = -\frac{1}{n}\sum_{i=1}^{n}\sum_{j=1}^{k} y_j^{(i)} \log(p_j^{(i)})
$$
...
$$
\mathrm{cost}(W) = -[y\log(H(X)) + (1-y)\log(1-H(X))]
$$
...
$$
\mathrm{cost}(W) = \frac{1}{n}\sum_{i=1}^{n}\sum_{j=1}^{k} y_j^{(i)} \times (-\log(p_j^{(i)}))
$$
...
6. 소프트맥스 회귀의 가설식은 $H(X)=\mathrm{softmax}(XW+B)$이다.
...
11. 소프트맥스 회귀의 비용 함수는 $\mathrm{cost}(W) = -\sum_{j=1}^{k} y_j\log(p_j)$ 형태이다.
